...
var chartUserService = function(chart, utils) {
	...
	return {
		...
		draw: function(data) {
			...
			if (parseInt(data.fuckCount) === 0) {
				...
				startA = -(Math.PI / 180) * (utils.calculateAngle(data.total, data.fuckCount, offsetAngle) + offsetAngle);
			}
			var pieData = [{startRadion: Math.PI, endRadion: 2 * Math.PI, pieColor: '#8ab14d'}, 
					{startRadion: startA, endRadion: endA, pieColor: '#e77037'}];

			var lineData = [{angle: utils.calculateAngle(data.total, data.fuckCount, offsetAngle) / 2, lineColor: '#e77037', text: data.fuckCount}];
...
chartUserService.$inject = ['chart', 'utils'];
