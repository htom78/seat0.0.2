...
var wordsPlace = function(mapService, $http, $templateCache, $compile, $document, $timeout, seatMapService, utils) {
	return {
		scope: {
			words: '=wordsPlace',
			startList: '='
							...
							mapService
								...
								.then(function(response) {
									if (parseInt(response.lng) !== 0 ) {
										...
										//火星转gps
										var gcj2gps = utils.gcj02ToGps84(response.lng, response.lat);
										mapService
											.getNearCars(gcj2gps.lng + ',' + gcj2gps.lat)
											.then(function(response) {
...
wordsPlace.$inject = ['mapService', '$http', '$templateCache', '$compile', '$document', '$timeout', 'seatMapService', 'utils'];
